...
public class SurvivalRadiusConfig {
    private static final String fileName = "/Others/SurvivalRadius.yml";
    private static File file = null;
    private static FileConfiguration secondConfig = null;

    public static FileConfiguration TienNang(Plugin plugin) {
        return SurvivalRadiusConfig.getConfig(plugin);
    }

    public static void setUpConfig(Plugin plugin) {
        file = new File(plugin.getDataFolder(), "/Others/SurvivalRadius.yml");
        if (!file.exists()) {
            try {
                file.createNewFile();
            }
    ...
    public static FileConfiguration getConfig(Plugin plugin) {
        if (secondConfig == null) {
            SurvivalRadiusConfig.setUpConfig(plugin);
        }
        return secondConfig;
    }
}
