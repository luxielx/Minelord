...
public class SatThuongConfig {
    private static final String fileName = "/Points/SatThuong.yml";
    private static File file = null;
    private static FileConfiguration secondConfig = null;

    public static FileConfiguration TienNang(Plugin plugin) {
        return SatThuongConfig.getConfig(plugin);
    }

    public static void setUpConfig(Plugin plugin) {
        file = new File(plugin.getDataFolder(), "/Points/SatThuong.yml");
        if (!file.exists()) {
            try {
                file.createNewFile();
            }
    ...
    public static FileConfiguration getConfig(Plugin plugin) {
        if (secondConfig == null) {
            SatThuongConfig.setUpConfig(plugin);
        }
        return secondConfig;
    }
}
