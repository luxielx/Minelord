...
public class LeveledItemDropConfig {
    private static final String fileName = "/Items/LeveledItemDrop.yml";
    private static File file = null;
    private static FileConfiguration secondConfig = null;

    public static FileConfiguration TienNang(Plugin plugin) {
        return LeveledItemDropConfig.getConfig(plugin);
    }

    public static void setUpConfig(Plugin plugin) {
        file = new File(plugin.getDataFolder(), "/Items/LeveledItemDrop.yml");
        if (!file.exists()) {
            try {
                file.createNewFile();
            }
    ...
    public static FileConfiguration getConfig(Plugin plugin) {
        if (secondConfig == null) {
            LeveledItemDropConfig.setUpConfig(plugin);
        }
        return secondConfig;
    }
}
